...
Specifically:
    ...
    - At the start of the game, when randomly selecting an answer from the list of possible answers, this circuit is used to randomly generate a corresponding list index.
...
Eg.
    - When choosing between two guesses' clues, there are only two options, which means the circuit needs to generate a either 0 and 1. If we want to represnt an arbitrary decimal value `decimal` as a binary number, we need `floor(log_2(decimal)) + 1` bits. Thus, to generate a number between 0 and 1, it is sufficient to create a circuit with  `floor(log_2(1)) + 1 = 1` qubit.
